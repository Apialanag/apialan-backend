...
router.post('/validar', async (req, res) => {
  ...
  if (!codigo_cupon || monto_neto_base_reserva === undefined) {
    return res.status(400).json({
      esValido: false,
      mensaje:
        'El código de cupón y el monto neto base de la reserva son requeridos.',
    });
  }

  if (
    typeof monto_neto_base_reserva !== 'number' ||
    monto_neto_base_reserva < 0
  ) {
    return res.status(400).json({
      esValido: false,
  ...
  try {
    ...
    if (cuponResult.rows.length === 0) {
      return res
        .status(404)
        .json({ esValido: false, mensaje: 'Cupón no encontrado.' });
    }
    ...
    if (!cupon.activo) {
      return res
        .status(400)
        .json({ esValido: false, mensaje: 'Este cupón ya no está activo.' });
    }
    ...
    if (cupon.fecha_validez_desde) {
      ...
      if (hoy < fechaDesde) {
        return res
          ...
          .json({
            esValido: false,
            mensaje: `Este cupón es válido a partir del ${format(
              fechaDesde,
              'dd/MM/yyyy'
            )}.`,
          });
    ...
    if (cupon.fecha_validez_hasta) {
      ...
      if (hoy > fechaHasta) {
        return res
          .status(400)
          .json({ esValido: false, mensaje: 'Este cupón ha expirado.' });
      }
    }
    ...
    if (monto_neto_base_reserva < parseFloat(cupon.monto_minimo_reserva_neto)) {
      return res.status(400).json({
        esValido: false,
        mensaje: `Este cupón requiere un monto mínimo de reserva de ${parseFloat(
          cupon.monto_minimo_reserva_neto
    ...
    if (cupon.tipo_descuento === 'porcentaje') {
      montoDescontado =
        (monto_neto_base_reserva * parseFloat(cupon.valor_descuento)) / 100;
    ...
    if (cupon.tipo_descuento === 'porcentaje') {
      ...
      mensajeExito += `${montoDescontado.toLocaleString('es-CL', {
        style: 'currency',
        currency: 'CLP',
      })} de descuento.`;
    }
    ...
    res
      ...
      .json({
        esValido: false,
        mensaje: 'Error interno del servidor al validar el cupón.',
      });
  }
});
...
router.post('/', async (req, res) => {
  const {
    ...
    un_solo_uso_por_socio = false, // Default false
  } = req.body;
  ...
  if (!codigo || !tipo_descuento || valor_descuento === undefined) {
    return res
      .status(400)
      .json({
        error: 'Código, tipo de descuento y valor de descuento son requeridos.',
      });
  }
  if (!['porcentaje', 'fijo'].includes(tipo_descuento)) {
    return res
      .status(400)
      .json({ error: "Tipo de descuento debe ser 'porcentaje' o 'fijo'." });
  }
  if (typeof valor_descuento !== 'number' || valor_descuento <= 0) {
    return res
      .status(400)
      .json({ error: 'Valor de descuento debe ser un número positivo.' });
  }
  if (tipo_descuento === 'porcentaje' && valor_descuento > 100) {
    // Porcentaje no puede ser > 100
    return res
      .status(400)
      .json({ error: 'Valor de descuento porcentual no puede exceder 100.' });
  }
  if (fecha_validez_desde && !isValid(parseISO(fecha_validez_desde))) {
    return res
      .status(400)
      .json({ error: 'Fecha de validez desde no es válida.' });
  }
  if (fecha_validez_hasta && !isValid(parseISO(fecha_validez_hasta))) {
    return res
      .status(400)
      .json({ error: 'Fecha de validez hasta no es válida.' });
  }
  if (
    fecha_validez_desde &&
    fecha_validez_hasta &&
    parseISO(fecha_validez_hasta) < parseISO(fecha_validez_desde)
  ) {
    return res
      .status(400)
      .json({
        error: 'Fecha de validez hasta no puede ser anterior a la fecha desde.',
      });
  }
  ...
  try {
    const nuevoCupon = await pool.query(
      `INSERT INTO cupones (
        ...
        descripcion, activo, usos_actuales, un_solo_uso_por_socio
    ...
    if (error.code === '23505') {
      // Error de violación de unicidad (ej. código duplicado)
      return res.status(409).json({ error: 'El código de cupón ya existe.' });
    }
    res
      .status(500)
      .json({ error: 'Error interno del servidor al crear el cupón.' });
  }
});
...
router.get('/', async (req, res) => {
  ...
  const countQuery = `SELECT COUNT(*) FROM cupones ${
  ...
  try {
    ...
    ); // Solo params de where para count
...
router.put('/:id', async (req, res) => {
  ...
  if (
    ...
    (typeof valor_descuento !== 'number' || valor_descuento <= 0)
  ) {
    return res
      .status(400)
      .json({ error: 'Valor de descuento debe ser un número positivo.' });
  ...
  try {
    ...
    if (tipo_descuento !== undefined)
      fieldsToUpdate.tipo_descuento = tipo_descuento;
    if (valor_descuento !== undefined)
      fieldsToUpdate.valor_descuento = valor_descuento;
    if (fecha_validez_desde !== undefined)
      fieldsToUpdate.fecha_validez_desde = fecha_validez_desde || null;
    if (fecha_validez_hasta !== undefined)
      fieldsToUpdate.fecha_validez_hasta = fecha_validez_hasta || null;
    if (usos_maximos !== undefined)
      fieldsToUpdate.usos_maximos = usos_maximos || null;
    if (monto_minimo_reserva_neto !== undefined)
      fieldsToUpdate.monto_minimo_reserva_neto = monto_minimo_reserva_neto || 0;
    ...
    if (usos_actuales !== undefined)
      fieldsToUpdate.usos_actuales = usos_actuales;
    if (un_solo_uso_por_socio !== undefined)
      fieldsToUpdate.un_solo_uso_por_socio = un_solo_uso_por_socio;

    // Construir las cláusulas SET parametrizadas (a partir del único fieldsToUpdate)
    const parametrizedSetClauses = Object.keys(fieldsToUpdate).map(
    ...
    if (Object.keys(fieldsToUpdate).length === 0) {
      ...
      return res
        ...
        .json({
          error:
            'No se proporcionaron campos (aparte de la fecha de actualización automática) para actualizar.',
        });
    }
    ...
    if (updatedCupon.rows.length === 0) {
      return res
        .status(404)
        .json({ error: 'Cupón no encontrado para actualizar.' });
    }
    ...
    if (error.code === '23505') {
      return res
        .status(409)
        .json({ error: 'El código de cupón ya existe para otro cupón.' });
    }
    res
      .status(500)
      .json({ error: 'Error interno del servidor al actualizar el cupón.' });
  }
});
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    if (desactivarCupon.rows.length === 0) {
      return res
        .status(404)
        .json({ error: 'Cupón no encontrado para desactivar.' });
    }
    res
      .status(200)
      .json({
        message: 'Cupón desactivado exitosamente.',
        cupon: desactivarCupon.rows[0],
      });
  } catch (error) {
    console.error(`Error al desactivar cupón ${id}:`, error);
    res
      .status(500)
      .json({ error: 'Error interno del servidor al desactivar el cupón.' });
  }
});
